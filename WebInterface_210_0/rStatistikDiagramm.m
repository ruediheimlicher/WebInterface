...
@implementation rStatistikDiagramm
...
- (id)initWithFrame:(NSRect)frame {
	...
	//	Diagrammfeld.size.height-=200;
		...
		//FaktorY=(frame.size.height-55.0)/255.0; // Reduktion auf Feldhoehe
	...
	[StatistikEinheitenDic setObject:[NSNumber numberWithInt:30]forKey:@"maxy"];
	[StatistikEinheitenDic setObject:[NSNumber numberWithInt:-10]forKey:@"miny"];
...
//			[GraphKanalArray addObject:[NSNumber numberWithInt:0]];
		...
		*/
		...
		//NSLog(@"TemperaturStatistikDiagramm init");
...
- (void)setWerteArray:(NSArray*)derWerteArray mitKanalArray:(NSArray*)derKanalArray
{
	...
	float	maxSortenwert=100.0;	// Temperatur, 100° entspricht 200 im Eingabewert bei Temperaturangabe vom Atmega
	...
	for (i=0;i<[derWerteArray count]-1; i++)
	{
		if ([[derKanalArray objectAtIndex:i]intValue])
		{
			...
			NSPoint neuerFusspunkt=neuerPunkt; // verwendet fuer Saeulen
			//NSLog(@"Diagramm neuerPunkt %d StartwertX: %2.2f x: %2.2f",i,StartwertX,neuerPunkt.x);
			float InputZahl=[[derWerteArray objectAtIndex:i+1]floatValue];	// Input vom IOW, 0-255
			//NSLog(@"setWerteArray FaktorY: %2.2f",FaktorY);
//			float graphZahl=(InputZahl-2*MinY)*FaktorY;							// Red auf reale Diagrammhoehe bei Eingaben vom Atmega, 
																								// Faktor 2 wegen doppeltem Wert der Eingabe bei Temperaturmessungen
			...
			*/
			...
			neuerPunkt.y += DiagrammWert;			// Ordinate des Wertes
			NSPoint neuerEndpunkt=neuerPunkt;	// Endpunkt bei Liniendarstellung
			neuerEndpunkt.x +=10;					// Endpunkt bei waagrechten Balkenstuecken
			
			
			NSString* tempWertString=[NSString stringWithFormat:@"%2.2f",InputZahl];///2.0]; // Anzeige im letzten Datenfeld
			...
			*/ 
			...
			NSRect n=NSMakeRect(neuerPunkt.x-2.0, neuerPunkt.y-2.0, 4,4);
			//[neuerGraph setLineWidth:1.0];
			[neuerGraph moveToPoint:neuerPunkt];
