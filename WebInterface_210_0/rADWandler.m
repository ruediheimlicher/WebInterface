...
//	[self Alert:@"ADWandler awake: vor EinkanalOrdinate"];
	...
	[[MKWertView textStorage]appendAttributedString:MKTabellenKopfString];
...
- (IBAction)reportRead1Kanal:(id)sender
{
	...
	float ZeitKompression=[[EinkanalZeitKompressionTaste titleOfSelectedItem]floatValue];
	if (Kanal<0)
	{
		NSLog(@"Kein Kanal");
		return;
...
- (IBAction)reportTrack1Kanal:(id)sender
{
	...
	int Kanal=[KanalWahlTaste selectedSegment];
	float ZeitKompression=[[EinkanalZeitKompressionTaste titleOfSelectedItem]floatValue];
	NSLog(@"ZeitKompression: %2.2f",ZeitKompression);
	if (Kanal<0)
	{
	NSLog(@"Kein Kanal");
...
- (void)readAllTimerFunktion:(NSTimer*)derTimer
{
	if (readAllIndex<8)
	{
		...
		[NotificationDic setObject:[NSNumber numberWithFloat:1.0] forKey:@"zeitkompression"];
		[NotificationDic setObject:[NSNumber numberWithInt:0] forKey:@"state"];
...
- (IBAction)reportTrack8Kanal:(id)sender
{
	...
	float ZeitKompression=[[MehrkanalZeitKompressionTaste titleOfSelectedItem]floatValue];
	NSLog(@"ZeitKompression: %2.2f",ZeitKompression);
	if (Kanal<0)
	{
	NSLog(@"Kein Kanal");
...
- (void)sendReadBitTimerFunktion:(NSTimer*)derTimer
{
	...
	[tempInputDic setObject:[[derTimer userInfo]objectForKey:@"zeitkompression"] forKey:@"zeitkompression"];//zeitkompression
...
//	NSLog(@"setEinkanalDaten: %@ Zeit: %@ Kanal: %d",dieDaten, dieZeit, derKanal );
	...
	{
		ZeitKompression=[[derVorgabenDic objectForKey:@"zeitkompression"]floatValue];
	}
	//NSLog(@"setEinkanalDaten ZeitKompression: %2.2f",ZeitKompression);
	...
	if ([tempDatenArray count])//schon Daten im Array
	{
		...
		tempZeit=[dieZeit timeIntervalSinceDate:DatenserieStartZeit]*ZeitKompression;		
	}
...
- (void)setMehrkanalDaten:(NSString*)dieDaten zurZeit:(NSDate*)dieZeit forKanal:(int)derKanal mitVorgaben:(NSDictionary*)derVorgabenDic
{
	...
	ZeitKompression=[[MehrkanalZeitKompressionTaste titleOfSelectedItem]floatValue];
	...
	{
		//NSLog(@"setMehrkanalDaten ZeitKompression: %2.2f",ZeitKompression);
...
//		NSString* tempWertString=[DatenFormatter stringFromNumber:[NSNumber numberWithFloat:y]];		
		...
		[tempDatenArray addObject:tempKanalDatenArray];
			...
			float rest=tempFrame.size.width-tempZeit*ZeitKompression;
			if (rest<100)
			{
				//NSLog(@"rest zu klein",rest);
				if (rest<0)//Wert hat nicht Platz
				{
					tempFrame.size.width=(tempZeit*ZeitKompression+100);
					tempOrigin.x=(tempZeit*ZeitKompression-100);
				}
				else
				{
					tempFrame.size.width+=200;
...
//	[Warnung addButtonWithTitle:@"Abbrechen"];
	...
	{
		//NSLog(@"ADWandler setEinkanalDaten tempDatenArray: %@",[tempDatenArray description]);
		// Zeit seit Start der Messungen (Ticks!)
		tempZeit=[[NSDate date] timeIntervalSinceDate:DatenserieStartZeit]*ZeitKompression;
